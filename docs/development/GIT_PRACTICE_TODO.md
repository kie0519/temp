...
### 23. 仓库清理与优化
- [x] 查看仓库大小（使用 du 和 git count-objects）
- [x] 创建临时文件演示清理操作
- [x] 使用 git clean 清理未跟踪的文件
- [x] 运行 git gc 进行垃圾回收优化
- [x] 使用 git fsck 验证仓库完整性

**实践结果：**
- 优化前：202 个松散对象，189.92 KiB
- 优化后：全部打包，72.32 KiB（节省 61.9%）
...
git gc                    # 垃圾回收
git fsck --full           # 完整检查仓库
```

### 24. 查找问题提交
- [x] 使用 git blame 查找每行代码的作者和提交
- [x] 使用 git blame -L 查看特定行范围
- [x] 使用 git grep 搜索所有文件中的代码模式
- [x] 使用 git grep -n 显示行号
- [x] 使用 git grep -c 统计匹配次数
- [x] 使用 git bisect 二分查找引入问题的提交

**实践成果：**
- 使用 blame 追踪了 hello.py 的修改历史
- 使用 grep 找到所有函数定义（12 个函数）
- 使用 bisect 成功定位到引入 power 函数的提交

**学到的命令：**
```bash
git blame <file>                  # 查看每行代码的作者
git blame -L 10,20 <file>         # 查看特定行范围
git grep "pattern"                # 搜索代码
git grep -n "pattern"             # 显示行号
git grep -c "pattern" -- "*.py"   # 统计并过滤文件
